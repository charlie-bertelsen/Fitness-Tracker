...
def run_etl():
    ...
    for filename in os.listdir("Data/Data_To_Upload"):
        ...
        if filename.lower().startswith("nutrition"):
            ...
            sleep_df['entry_date'] = pd.to_datetime(sleep_df_raw['Sleep Score 4 Weeks']).dt.date
            ...
            weight_df = load_csv(file_path)
            weight_df.rename(columns=lambda x: x.strip(), inplace=True)
            weight_df['user_id'] = user_id

            # Extract and forward-fill the date
            weight_df['date'] = weight_df['Time'].where(weight_df['Time'].str.contains("2025", na=False))
            weight_df['date'] = weight_df['date'].ffill()
            weight_df = weight_df.drop(columns=['Time'])

            # Drop rows with missing weight
            weight_df = weight_df[weight_df['Weight'].notna()]

            # Keep only relevant columns and rename
            weight_df = weight_df[['user_id', 'date', 'Weight', 'Change', 'BMI']]
            weight_df.rename(columns={
                'Weight': 'weight_lbs',
                'Change': 'change_lbs',
                'BMI': 'bmi'
            }, inplace=True)

            weight_df = weight_df.convert_dtypes()
            weight_df['date'] = pd.to_datetime(weight_df['date'].str.strip(), format='%b %d, %Y', errors='coerce')
            weight_df['weight_lbs'] = weight_df['weight_lbs'].str.replace(' lbs', '', regex=False).astype(float)
            weight_df['change_lbs'] = weight_df['change_lbs'].str.replace(' lbs', '', regex=False).astype(float)
            weight_df.to_sql('weight', con=engine, if_exists='append', index=False)
            print("✅ Weight data uploaded.")
